...
```{r mult model 2}
mult_model_2 <- gls(
  bprs ~ age + edu + bmi + inkomen + adl + wzc + cdrsb_base + ab_base +
    (sex + job) * year ,
  correlation = corAR1(form = ~ year | sample),  # struttura UN
  method = "ML",
  data = alz_long,
...
```{r}
alz_long$adl <- as.numeric(alz_long$adl)
alz_long$year <- as.numeric(alz_long$year)
alz_long$year_fac <- as.factor(alz_long$year)

mult_model_1 <- gls(
  bprs ~ 
    (sex + job + age + edu + bmi + inkomen + adl + wzc + cdrsb_base + ab_base) * year ,
  correlation = corAR1(form = ~ year | sample),  # struttura UN
  weights = varIdent(form = ~ 1 | year),  # eterogeneità delle varianze
  method = "ML",
  data = alz_long,
...
```{r}
mean_df$adl <- as.numeric(mean_df$adl)
mean_df$pred_multi_1 <- predict(mult_model_1, mean_df)
...
mult_model_2 <- gls(
  bprs ~ 
    (sex + job + age + inkomen + adl + wzc + ab_base + tau_base) * year ,
  correlation = corAR1(form = ~ year | sample),  # struttura UN
  weights = varIdent(form = ~ 1 | year),  # eterogeneità delle varianze
  method = "ML",
  data = alz_long,
...
mult_model_UN <- gls(
  bprs ~ 
    (sex + job + age + inkomen + adl + wzc + ab_base + tau_base) * year,
  correlation = corSymm(form = ~ 1 | sample),   # correlazione non strutturata (UN)
  weights     = varIdent(form = ~ 1 | year),    # varianze eterogenee per ogni tempo
  method      = "ML",
  data        = alz_long,
  na.action   = na.exclude
)

anova(mult_model_1, mult_model_UN)
```
